...
public class DBStore implements AutoCloseable, Store {
    ...
    public boolean updateUser(int id, User user) {
        try (Connection connection = SOURCE.getConnection()) {
            String sql = "update users set name = ?, login = ?, password = ?, email = ?," +
                    " createdate = ?, role = ?, country = ?, city = ? where id = ?";
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
            preparedStatement.setString(1, user.getName());
            preparedStatement.setString(2, user.getLogin());
            preparedStatement.setString(3, user.getPassword());
    ...
    public static void main(String[] args) {
        getInstance();
        DBStore.getInstance().addUser(new User("Ivan", "vano", "111", "dddd", "1111", "admin", "Russia", "Msk"));
